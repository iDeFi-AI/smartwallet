...
export function NavbarLink({
  ...
  return (
    <NextLink
      href={href}
      className="font-robotoMono px-0 text-center text-base font-normal text-black no-underline"
      target={target}
...
export function NavbarTitle() {
  return (
    <div className="flex h-8 items-center justify-start gap-4">
      <NextLink href="/" passHref className="relative h-12 w-12" aria-label="Home page">
        <Image
          src="/icons/48x48.png" // Path to your logo image
          alt="iDEFi.AI Logo"
          width={52} // Set width as needed
      ...
      <NextLink
        href="/"
        passHref
        className="font-robotoMono text-center text-xl font-medium text-black no-underline"
        aria-label="build-onchain-apps Github repository"
      >
        iDEFi.AI CRYPTO AGENT
      </NextLink>
    </div>
  );
}
                ...
                <NavigationMenu.List className={clsx('flex flex-row space-x-2')}>
                  <NavigationMenu.Item>
                    <NavigationMenu.Trigger className="group flex h-16 items-center justify-start gap-1">
                      <span className="font-robotoMono text-center text-base font-normal text-black">
                        Experiences
                      </span>
                      <ChevronDownIcon
